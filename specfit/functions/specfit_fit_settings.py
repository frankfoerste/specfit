...
class SpecFitFitSettings(QtWidgets.QWidget):
    def __init__(self, parent=None, _parent=None):
        ...
        #: time to check if mouse was double clicked
        self.time_press = time.time()
        #: variable used to switch between 'local' and 'global' minimum
        # optimization
        self.minimum = 'local'
        #: variable used to decide if iteration results should be plotted or
        # not
        self.plot_iterations = True
        #: list of colors used for plotting
        self.colors = list(col.TABLEAU_COLORS.values())*10
    ...
    def __init__UI(self):
        ...
        self.check_save_background = QtWidgets.QCheckBox(
            'save background', self)
        self.check_save_fitted_spectrum = QtWidgets.QCheckBox(
            'save fitted spectrum', self)
        ...
        self.check_save_background.setCheckState(
    ...
    def __init_connections(self, ):
        self.entry_a0.textChanged.connect(
            partial(self.update_entries, self.parent.entry_a0))
        self.entry_a1.textChanged.connect(
            partial(self.update_entries, self.parent.entry_a1))
        self.entry_fano.textChanged.connect(
            partial(self.update_entries, self.parent.entry_fano))
        self.entry_FWHM.textChanged.connect(
            partial(self.update_entries, self.parent.entry_FWHM))
        self.entry_strip_cycles.textChanged.connect(
            partial(self.update_entries, self.parent.entry_strip_cycles))
        self.entry_strip_width.textChanged.connect(
            partial(self.update_entries, self.parent.entry_strip_width))
        self.entry_smooth_cycles.textChanged.connect(
            partial(self.update_entries, self.parent.entry_smooth_cycles))
