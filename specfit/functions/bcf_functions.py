...
def bcf2spec_para(
    ...
    a0 = data.original_metadata["Spectrum"]["CalibAbs"]
    a1 = data.original_metadata["Spectrum"]["CalibLin"]
    fwhm = 2*np.sqrt(2*np.log(2))*np.sqrt(
        data.original_metadata["Spectrum"]["SigmaAbs"])
    fano = data.original_metadata["Spectrum"]["SigmaLin"]/(3.85e-3)
    channels = data.original_metadata["Spectrum"]["ChannelCount"]
    gating_time = 3e-6
    real_time = data.metadata["Acquisition_instrument"]["SEM"][
        "Detector"]["EDS"]["real_time"]
    ...
    ds["tensor positions"] = xr.DataArray(
        data=np.column_stack((
            row_indices.flatten(),
            col_indices.flatten(),
            depth_indices.flatten())),
        dims=("spec_nr", "dimension")
        )
    ds["positions"] = xr.DataArray(
        data=ds["tensor positions"]*[dx, dy, 1]+[X0, Y0, Z0],
        dims=("spec_nr", "dimension"),
    ...
    ds["spectra"] = xr.DataArray(
        ...
        dims=("X", "Y", "Z", "energy"),
        coords={
            "energy": np.arange(a0, a0 + a1*(channels), a1),
            "X": np.linspace(
                start=X0,
                stop=X0+dx*data.data.shape[0]-dx,
                num=data.data.shape[0]),
            "Y": np.linspace(
                start=Y0,
                stop=Y0+dy*data.data.shape[1]-dy,
    ...
    if return_values:
        return (
            ds["spectra"],
            ds["parameters"],
            ds["position dimension"],
            ds["tensor positions"],
            ds["sum spec"]
        )


def bcf2spec_para_dask(
        folder_path,
        verbose=True
        ):
    """
    ...
    row_indices, col_indices, depth_indices = np.indices(size, dtype=np.uint)
    tensor_positions = da.from_array(
        np.column_stack((
            row_indices.flatten(),
            col_indices.flatten(),
            depth_indices.flatten())),
    ...
    spectra = spectra.reshape([
        ...
        channels])
    spectra = da.asarray(
        spectra.reshape(nr_spectra, channels).astype(np.float64),
    ...
    # save everything to a data h5 file
...
def many_bcf2spec_para(
        folder_path,
        return_values=False,
        verbose=False
        ):
    """
    This function reads out all the .bcf-files in the inputted folder_path and
    reads out the detector parameters given in the .bcf-files.
    ...
    # creates a list with all .bcf-files stored inside the folder
    bcf_file_list = [file for file in folder_path.glob("*.bcf")]
    ...
    ds = xr.Dataset()
    for file_nr, file_path in enumerate(bcf_file_list):
        data = hs.load(file_path, lazy=True,
                       select_type="spectrum_image",
    ...
    for file_nr, file_path in enumerate(bcf_file_list):
            ...
            if dz != 0:
                spectra = np.zeros(
                    shape=data.data.shape[:-1]+(
                        nr_bcf_files,)+(data.data.shape[-1],))
                shape = spectra.shape
            else:
                spectra = np.zeros(
                    shape=data.data.shape[:-1]+(1,)+(data.data.shape[-1],))
                shape = spectra.shape
        ...
        if file_nr == 0:
            if dz != 0:
                ...
                ds["position dimension"] = xr.DataArray(
                    ...
                    dims=("dimension"))
            ds["tensor positions"] = xr.DataArray(
                data=np.asarray(list(itertools.product(
                    np.arange(ds["position dimension"][0]),
                    np.arange(ds["position dimension"][1]),
                    np.arange(ds["position dimension"][2]))),
                    dtype=np.uint),
                dims=("spec_nr", "dimension"),
                coords={
                    "spec_nr": np.arange(np.prod(ds["position dimension"]))})
        life_times = data.data[..., 75:116].sum(axis=-1)*1e-4
        life_time_sum = life_times.mean()

        # some maps do not contain spectra, this leads to a life-time of 0,
        # skip them
        if life_time_sum == 0:
            continue
        ...
        if dz != 0:
            ...
            spectra += (data.data/life_times[..., np.newaxis]).reshape(
                shape).compute()

            if file_nr != 0:
                spectra /= 2

        parameters_temporary = np.array([
            a0,
            a1,
            fano,
            fwhm,
    ...
    ds["parameters"] = xr.DataArray(
        data=parameters/len(bcf_file_list),
        dims=("parameter"),
        coords={
            "parameter": [
                "a0",
    ...
    if dz != 0:
        ds["spectra"] = xr.DataArray(
            ...
            coords={
                ...
                "X": np.linspace(
                    start=X0,
                    stop=X0 + dx * ds["position dimension"][0] - dx,
                    num=data.data.shape[0]),
                "Y": np.linspace(
                    start=Y0,
                    stop=Y0 + dy * ds["position dimension"][1] - dy,
                    num=data.data.shape[1]),
                "Z": np.linspace(
                    start=Z0,
                    stop=Z0 + dz * ds["position dimension"][2] - dy,
                    num=data.data.shape[1])},
            attrs={"units": "counts per second"})
    else:
        ds["spectra"] = xr.DataArray(
    ...
    ds["sum spec"] = ds["spectra"].mean(axis=(0, 1, 2))
    ds["positions"] = xr.DataArray(
        data=ds["tensor positions"] * [dx, dy, dz] + [X0, Y0, Z0],
        dims=("spec_nr", "dimension"),
        coords={"dimension": ["x", "y", "z"]},
        attrs={"units": ["mm", "mm", "mm"]})
...
def bcf_tensor_position(file_path):
    ...
    tensor_position = np.asarray(list(itertools.product(
        range(position_dim[0]),
        range(position_dim[1]))),
        dtype=np.uint)
    tensor_position = np.hstack((
        tensor_position,
        np.zeros(
            (len(tensor_position), 1),
            dtype=np.uint)))
