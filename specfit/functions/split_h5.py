...
def split_h5():
    ...
    with h5py.File(data_path, 'r') as f:
        ...
        # create a list of splits from the row_split and first dimension of
        # the data
        if dimensions[0]%rows_split != 0:
            list_of_splits = [rows_split]*(
                 dimensions[0]//rows_split)+[dimensions[0]%rows_split]
        else:
        ...
        for i, split in enumerate(list_of_splits):
            # calculate start and end index, since the data is stored as one
            # long list (e.g. np.prod(dimensions)), the index has to be
            # calculated accordingly
            start_idx = np.sum(
                 list_of_splits[:i] * np.prod(dimensions[1:]), dtype=int)
            end_idx = np.sum(
                 list_of_splits[:i+1] * np.prod(dimensions[1:]), dtype=int)
            ...
            with h5py.File(split_name, 'w') as split_file:
                    # Create the datasets
                    split_file.create_dataset(
                         'counts',
                         data=f['counts'][start_idx:end_idx])
                    split_file.create_dataset(
                         'max pixel spec',
                         data=f['max pixel spec'])
                    if f['parameters'].ndim == 1:
                        ...
                        split_file.create_dataset(
                             'parameters',
                             data=f['parameters'][start_idx:end_idx])
                    split_file.create_dataset(
                         'position dimension',
                         data=split_dimensions)
                    split_file.create_dataset(
                         'positions',
                         data=split_positions)
                    split_file.create_dataset(
                         'spectra',
                         data=f['spectra'][start_idx:end_idx])
                    split_file.create_dataset(
                         'sum spec',
                         data=f['sum spec'])
                    split_file.create_dataset(
