...
import specfit.functions.spx_functions as spx
import specfit.functions.spe_functions as spe
import specfit.functions.mca_functions as mca
import specfit.functions.msa_functions as msa
import specfit.functions.txt_functions as txt
import specfit.functions.hdf_functions as hdf
import specfit.functions.bcf_functions as bcf
import specfit.functions.csv_functions as csv
import specfit.functions.angles_functions as angles
...
class SPXThread(QtCore.QThread):
    ...
    def run(self):
        spx.many_spx2spec_para(folder_path=self.folder_path,
...
class DataHandler():
    ...
    def reset_2_default(self):
        # if this file is in the same folder as spectra file/dir it is
        ...
        # stores tensor positions [[x1,y1,z1],[x2,y1,z1], ..., [xn,yn,zn]]
        ...
        self.threshold = 1e-3
        # nested list with [[name,start,end],[name,start,end]]
        self.user_defined_lines = []
        self.log_file_type = None  # 'spx or MSA','Louvre','None'
        # stores width,start, end as defined in log file
        # [[scan_width],[start],[end],[position_dim]] = spx_log_content(path)
        self.log_content = []
        self.file_dialog = None
        self.main_thread = None
        self.file_list = []  # list of all files of type
        self.mincount = 0  # spec with less counts in ROI are not evaluated
        # label from the main GUI that can display the progress of the loading
        # process
    ...
    def _get_folder_path(self):
        ...
        try:
            ...
            self.label_loading_progress.showMessage(str(folderpath))
            assert Path(folderpath).exists()
        except:
            self.label_loading_progress.showMessage("Dir not found")
            folderpath = Path("not_found")
        return folderpath
    ...
    def check_h5(self, ):
        """
        Function to check if the file_name is inside of the data.h5
        """
    ...
    def get_reload(self, ):
        ...
        """
        reload = QtWidgets.QMessageBox.question(
            self.SpecFit_MainWindow, "?",
            "Do you want to reload the measurement?",
            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
            QtWidgets.QMessageBox.No)
        if reload == QtWidgets.QMessageBox.No:
    ...
    def update_data_from_h5file(self, h5_path):
        ...
        if self.position_dimension.ndim != 1:
            print(
                "self.position_dimension.ndim: ",
                self.position_dimension.ndim)
            # for mca measurements the tensor positions and positions need to
            # be reshaped to a list with subarrays
            if "len_scans" in dir(self):
                split_indices = np.cumsum(self.len_scans[:-1])
            else:
                lengths = np.prod(self.position_dimension, axis=1)
                split_indices = np.cumsum(lengths)[:-1]
            self.tensor_positions = np.split(
    ...
    def open_data_file(self,
        ...
        _reload : bool, optional
            ...
            by default True
        """
        self.file_path, self.loadtype = self._get_file_path(
        ...
        if self.file_type in [".msa", ".MSA"]:
            ...
            if (self.save_data_folder_path / "data.h5").is_file() and self.check_h5():
                if _reload:
                    ...
                    if reload == QtWidgets.QMessageBox.Yes:
                        ...
                        print("something is wrong with .MSA worker,",
                              " parameters are not stored correctly")
            else:
                msa.msa2spec_sum_para(file_path=self.file_path)
                assert self.load_stored_spec_and_param()
                print(
                    "something is wrong with .MSA worker, parameters are not ",
                    "stored correctly")
            ...
            if (self.save_data_folder_path/"data.h5").is_file() and self.check_h5():
                if _reload:
                    reload = self.get_reload()
                    if reload == QtWidgets.QMessageBox.Yes:
                        self.spectra, self.parameters = csv.csv2spec_para(
                            self.file_path)
                        if self.parameters.ndim == 1:
                            self.parameters = np.expand_dims(
                                self.parameters,
                                axis=0)
                        self.tensor_positions = csv.csv_tensor_positions(
                            self.file_path)
                        self.positions = self.tensor_positions
                        self.position_dimension = csv.csv_position_dimension(
                            self.file_path)
            ...
            if self.file_type == ".spx":
                ...
                if self.save_data_folder_path.is_dir():
                    reload = self.get_reload()
                    if reload == QtWidgets.QMessageBox.Yes:
        ...
        param_path = self.folder_path / self.default_param_file_name
    ...
    def open_data_folder(self,
                ...
                if reload == QtWidgets.QMessageBox.Yes:
                    ...
                    with h5py.File(self.save_data_folder_path / "data.h5", "r+") as tofile:
                        ...
                        if f"{self.file_name}/position dimension" not in tofile:
                            tofile.create_dataset(
                                f"{self.file_name}/position dimension",
        ...
        self.real_time = self.parameters[0][7]
        self.create_save_folder()
        self.check_param_file(self.folder_path / self.default_param_file_name)
        self.update_channels()
        self.parent.entry_a0.setText(f"{self.parameters[0][0]}")
        self.parent.entry_a1.setText(f"{self.parameters[0][1]}")
        self.parent.entry_fano.setText(f"{self.parameters[0][2]}")
    ...
    def load_stored_spec_and_param(self, folder=False, stored_data=False):
        ...
        if (self.save_data_folder_path / "data.h5").is_file() and self.check_h5():
            ...
            stored_data = True
            with h5py.File(self.save_data_folder_path / "data.h5", "r") as infile:
        ...
        elif (self.save_data_folder_path / "parameters.npy").exists() and \
        (self.save_data_folder_path / "tensor_positions.npy").exists() and \
        (self.save_data_folder_path / "sum_spec.npy").exists() and \
        (self.save_data_folder_path / "position_dimension.npy").exists():
            self.label_loading_progress.showMessage("loading stored data")
            if (self.save_data_folder_path / "spectra.pickle").exists():
                stored_data = True
                if not folder:
                    self.spectra = utils.open_dict_pickle(
                ...
                self.len_spectrum = len(self.sum_spec)
            elif (self.folder_path / "single_spectra").is_dir():
                stored_data = True
    ...
    def build_tensor_position(self, positions):
        """
        needs list/array with [xmax,ymax,z_max]
    ...
    def run_file_worker(self):
        if self.file_type == ".bcf":
            ...
            if self.file_path in [".msa", ".MSA"]:
                worker = globals()[f"{self.file_type[1:].upper()}Thread"](self.file_path)
            else:
                worker = globals()[f"{self.file_type[1:].upper()}Thread"](self.folder_path)
                worker.signals.progress.connect(self.run_file_progress)
            worker.start()
            worker.wait()
    ...
    def save_npy_probs(self):
        ...
        """
        if not self.save_data_folder_path.exists():
            Path(self.save_data_folder_path).mkdir(parents=True, exist_ok=True)
        with h5py.File(self.save_data_folder_path / "data.h5", "a") as tofile:
            if "counts" in list(tofile.keys()):
                ...
                tofile.create_dataset(
                    f"{self.file_name}/counts",
                    data=self.counts)
                tofile.create_dataset(
                    f"{self.file_name}/parameters",
                    data=self.parameters)
                tofile.create_dataset(
                    f"{self.file_name}/sum spec",
                    data=self.sum_spec)
                tofile.create_dataset(
                    f"{self.file_name}/max pixel spec",
                    data=self.max_pixel_spec)
                tofile.create_dataset(
                    f"{self.file_name}/spectra",
                    data=self.spectra)
    ...
    def create_save_folder(self):
        if self.loadtype in ["file", "msa_file", "angle_file", "hdf5_file", "bcf_file",
            ...
            self.save_folder_path = f"{self.folder_path}/results"
        Path(self.save_folder_path).mkdir(parents=True, exist_ok=True)
        ...
        if self.log_file_type is None:
            if self.file_type == ".spx":
                ...
                if len(x) != 1:
                    ...
                    self.step_width[0] = np.ceil(
                        np.abs((x[0]-x[-1])/(len(x)-1)) * 1000) / 1000
                    tensor_position[:, 0] /= self.step_width[0]
                if len(y) != 1:
                    # self.step_width[1] = np.round(np.diff(y).mean(), 3)
                    self.step_width[1] = np.ceil(
                        np.abs((y[0]-y[-1])/(len(y)-1)) * 1000) / 1000
                    tensor_position[:, 1] /= self.step_width[1]
                if len(z) != 1:
                    # self.step_width[2] = np.round(np.diff(z).mean(), 3)
                    self.step_width[2] = np.ceil(
                        np.abs((z[0]-z[-1])/(len(z)-1)) * 1000) / 1000
                ...
                for scan in range(nr_scans):
                    ...
                    if np.ndim(positions[scan]) != 1:
                        ...
                        except np._core._exceptions._UFuncOutputCastingError as e:
                            print(e)
                            tensor_position[scan][:, _mask] = tensor_position[scan][:, _mask]/self.step_width[scan][_mask].astype(float)
                ...
                for scan in sorted(empty_scans, reverse=True):
                    ...
                    try:
                        if isinstance(self.len_scans, dict):
                            self.len_scans.pop(scan)
                        else:
                ...
                with h5py.File(self.save_data_folder_path / "data.h5", "r+") as tofile:
                    ...
                    del tofile[f"{self.file_name}/len scans"]
                    tofile.create_dataset(
                        f"{self.file_name}/len scans",
                        data=np.array(list(self.len_scans.values())),
                        compression="gzip")
            ...
            if self.log_file_type == "Louvre":
                ...
                for i, _ in enumerate(file_list):
                    ...
                    tensor_position[i] = spx.spx_position(file_list[i])
