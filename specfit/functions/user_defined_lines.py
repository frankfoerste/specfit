...
class LinesWidget(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        #: determine wether a spectrum was loaded or not
    ...
    def get_channel_of_energy(
            self,
            energy
            ):
        energy = np.array(energy).astype(float)
        return np.rint((np.divide(np.subtract(
            energy,
            self.a0), self.a1))).astype(int)

    def get_energy_of_channel(
            self,
            channel
            ):
    ...
    def add_new_line(self):
        ...
        self.button_remove_list.append(QtWidgets.QPushButton(
            QtGui.QIcon.fromTheme("window-close")," " ,self))
        ...
        self.button_remove_list[-1].clicked.connect(
            lambda: self.remove_line((ident)))
    ...
    def remove_line(
        ...
        for line_under_i in range(i, self.number_lines):
            for line_list in [
                    ...
                    self.entry_end_list,
                    self.button_remove_list]:
                line_list[line_under_i].move(
                    line_list[line_under_i].x(),
                    line_list[line_under_i].y()-30)
    ...
    def show_gaussian_fits(self):
        """
        plot the gaussian fits and the data in new windows to check if the fit
        looks ok
        """
        for popt,label,x in zip(self.popt_list,self.label_list,self.x_list):
            ...
            plt.plot(
                x,
                self.spec[self.get_channel_of_energy(x)],
                "b+:",
                label="data")
            plt.plot(
                x,
                self.gaussian(x,*popt),
                "ro:",
                label="fit")
            plt.legend()
            plt.show()
    ...
    def gaussian_fit(self):
        """
        try a gaussian fit for every user defined line, return the labels of
        sucessfully fitted lines and its parameters
        """
        self.popt_list = []
        self.label_list = []
        self.x_list = []
        for label_entry, start_entry, end_entry in zip(
                self.entry_label_list,
                self.entry_start_list,
                self.entry_end_list):
            if  start_entry.text() != "" and end_entry.text()!="":
                ...
                x = self.get_energy_of_channel(
                    np.array(range(start_ch, end_ch+1)))
                selected_area = self.spec[self.get_channel_of_energy(x)]
                mean = self.get_energy_of_channel(
                    start_ch + (-start_ch+end_ch)/2)
                sigma = 0.2
                ...
                try:
                    popt, pcov = curve_fit(
                        ...
                        x,
                        selected_area,
                        p0=[maximum,mean,sigma])
                    self.popt_list.append(popt)
                    self.label_list.append(label)
                    self.x_list.append(x)
                except RuntimeError:
                    print(
                        "gaussian fit not possible, ",
                        "take default parameters for ",
                        label)
    ...
    def gaussian_impossible_handler(self, label):
        """
        build a dict: linelabel -> [list of angles where gaussian parameters
        could not be found]
        """
        if self.spec_label != "test":
    ...
    def get_user_defined_lines_data(self):
         ...
         return (np.array((
             self.get_label_list(),
             self.get_start_list(),
             self.get_end_list())).T).tolist()

    def build_output_rows(self):
        user_lines_list = []
        try:
            for popt in self.popt_list:
                e = self.get_energy_of_channel(
                    np.array(range(0,len(self.spec))))
                line_spectra = self.gaussian(e, *popt)
                e0 = popt[1]
                sigma = popt[2]
                line_spectra[:min(
                    self.get_channel_of_energy(e0-4*sigma),
                    len(line_spectra))]= 0
                line_spectra[max(
                    self.get_channel_of_energy(e0+4*sigma),0):]= 0
    ...
    def plot_output(self,line_spectra_list):
         ...
         """
         for label,line_spectra,popt in zip(
                self.label_list,
                line_spectra_list,
                self.popt_list):
             a = popt[0]
    ...
    def print_gaussian_fit_error_results(self):
        if len(self.opt_param_not_found_dict.keys())>0:
            print("-"*66)
            for key, val in self.opt_param_not_found_dict.items():
                print(f"{key}: {len(self.opt_param_not_found_dict[key])}",
                      " spectra could not be fitted" )
                print(self.opt_param_not_found_dict[key])
            print("-"*66)
