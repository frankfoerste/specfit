...
##############################################################################
from user_defined_lines import lines_widget
...
class periodic_table(QtWidgets.QWidget):
    def __init__(self, 
                 parent = None, 
                 PSE = None, 
                 elements = None, 
        ...
        super(periodic_table, self).__init__(parent)
        try: QtGui.QIcon.setThemeName('ubuntu-mono-dark')
        except: print(QtGui.QIcon.themeName())
    ...
    def __init__line_finder_widget(self):
        ...
        self.tab_line_finder = lf.line_finder(parent = None, module_path = module_path)
    ...
    def element_list(self):
        ...
        for i in range(len(self.elements)):       
            ...
            # self.check_elements[i].clicked.connect(partial(self.read_element_choice,self.elements[i][1]))
    ...
    def create_tool_tip_element(self, element):
        element = str(element)
        tool_tip = ''
        for k in range(len(self.characteristic_lines[element])):
            tool_tip += '%s \t %s \n'%(self.characteristic_lines['No.'][k],
                                       self.characteristic_lines[element][k])
        return tool_tip
        ...
        if paramfile or not(element_str in self.selected_elements):
            ...
            if element < 13:
                ...
                self.ax_canvas_spectrum.text(Ka[0]+0.1,
                                            0.3,'%s - K'%element_str, rotation = 90, color = 'green',
                                            label = element_str)
            elif 13 <= element <= 39:
                Ka = [xrl.LineEnergy(element,xrl.KA1_LINE), xrl.RadRate(element, xrl.KA1_LINE)]
                ...
                self.ax_canvas_spectrum.text(xrl.LineEnergy(element,xrl.L3M5_LINE)+0.1,
                                            0.3,'%s - La'%element_str, rotation = 90,
                                            label = element_str)
            else:
                ...
                self.ax_canvas_spectrum.text(xrl.LineEnergy(element,xrl.M5N7_LINE)+0.1,
                                            0.3,'%s - Ma'%element_str, rotation = 90,
                                            label = element_str)
    ...
    def line_list(self, element_str):
        ...
        for i in range(1,14):
            self.check_lines_list[-1][i] = QtWidgets.QCheckBox('%s'%(self.lines[i-1][0]), self.element_widget)
    ...
    def Lines_separate(self, lineindex): #1:K 2:Ka 3:Kb
        ...
        for i in range(len(self.check_lines_list)):
            ### K
            if lineindex == 1:
                if self.check_lines_list[i][1].checkState() != 0:
                    self.check_lines_list[i][2].setChecked(False)
                    self.check_lines_list[i][3].setChecked(False)
            elif (lineindex == 2 or lineindex == 3):
                if self.check_lines_list[i][lineindex].checkState() != 0:
                    self.check_lines_list[i][1].setChecked(False)
            ### L
            elif lineindex == 4:
                if self.check_lines_list[i][4].checkState() != 0:
            ...
            ### M
                ...
                if self.check_lines_list[i][8].checkState() != 0:
...
#       if not any(k in linedict for k in linelist):
#           self.statusBar().showMessage("Unknown key. Maybe something's wrong in the param file?")
    ...
    def remove_line_list(self, element_str):
        ...
        for k in range(len(self.check_lines_list)): #find the correct position for all other
            for l in range(14):
                if l == 0:
        ...
        # for i in range(len(self.ax_canvas_spectrum.lines)):
        #     self.ax_canvas_spectrum
        try:
...
#        self.check_fit() #wenn man in der GUI Daten geändert hat soll er sie auch direkt anzeigen und speichern
    ...
    def build_specfit_addlines(self):
        ...
        for element, lines in zip(self.selected_elements, self.selected_lines):
            self.specfit_addlines['%s'%element] = (True, lines, xrl.SymbolToAtomicNumber(element))
