...
except: print('Please install hyperspy using "conda install hyperspy"\n or "python -m pip install hyperspy"')
# import codecs
import itertools
...
def bcf2spec_para(file_path, save_sum_spec=True, save_spectra=True, 
    ...
    data = hs.load(file_path, lazy=True, select_type='spectrum_image')
    ### the last entry contains the measurement data (the others images)
    ### read out parameter
    if verbose:
        print('### read out parameter')
...
def many_bcf2spec_para(folder_path, signal=None ,worth_fit_threshold=200,
    ...
    list containing the detector parameters [a0, a1, FANO, FWHM]
    '''
    try: os.mkdir('%s/data/'%folder_path)          # creates data folder in folder_path if not existing
    except: pass
    ...
    for file_nr, file_path in enumerate(bcf_file_list):          # iteration over all .bcf-files
        ...
        print('real_time from bcf', real_time)
        real_time /= nr_spectra
        ...
        spectra_temporary = {} # blank for storing spectra for one file
        life_time_sum = 0
        for i in range(nr_spectra):
            ...
            spectra_temporary['%i'%i] =  spectrum / life_time 
...
def bcf_tensor_position(file_path):
    ...
    bcf_file = hs.load(file_path, signal_type='EDS_SEM')
    if len(bcf_file) <= 5:
        data = bcf_file[4]
