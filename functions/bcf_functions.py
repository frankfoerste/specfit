...
try:
    ...
    print('Please install hyperspy using "conda install hyperspy"\n or "python -m pip install hyperspy"')
...
def many_bcf2spec_para(folder_path, signal=None ,worth_fit_threshold=200,
    ...
    """
    try:
        os.mkdir(f"{folder_path}/data/")          # creates data folder in folder_path if not existing
    except:
        pass
    bcf_file_list = glob(f"{folder_path}/*.bcf")   # creates a list with all .bcf-files stored inside the folder
