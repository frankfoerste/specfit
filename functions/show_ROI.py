...
class show_ROI(QtWidgets.QWidget):
    """
    ...
    def __init__(self, parent = None):
        ...
        for i in range(len(self.elements)):
            self.elements[i] = '%d - %s'%(self.elements[i][0], self.elements[i][1])
    ...
    def init_UI(self):
     ...
     #only visible if angle file is loaded 
      #  self.entry_position.textChanged.connect(self.entry_position_spectra)
        
        ### fill combobox with elements ###
        try: self.combo_elements.addItems(self.elements)
        except: pass
    
        try: self.combo_lines.addItems(self.lines)
        except: pass
    
        self.combo_rect_size.addItems(self.rect_sizes)
        # self.combo_elements.setModelColumn(6)
...
#        self.ax_canvas_matplot.set_title('Counts per second')
...
#        self.ax_canvas_matplot.set_xlim(0,40)
        self.ROI_plotted = False

    def display_show_ROI(self,):
        try: self.parent.data.file_type
        except: 
    ...
    def load_spectra(self, folder_path, save_folder_path, save_data_path, 
        ...
        with h5py.File(f'{self.save_data_path}/data.h5', 'r') as infile:
            self.spectra = da.asarray(infile['spectra'][()])
            self.angles = infile['positions'][()]
            self.positions = infile['positions'][()]
            self.tensor_positions = infile['tensor positions'][()]
            self.parameters = infile['parameters'][()]
            self.counts = infile['counts'][()]
            self.len_x, self.len_y, self.len_z = infile['position dimension'][()]
        ...
        if one_dim: 
            ...
            self.log_box.setVisible(False)
            self.one_dim= False
            with h5py.File('%s/data.h5'%self.save_data_path, 'r') as infile:
                self.positions = infile['positions'][()]
                self.tensor_positions = infile['tensor positions'][()]
                self.counts = infile['counts'][()]
                self.parameters = infile['parameters'][()]
        ...
        # self.tensor_positions = np.load('%s/tensor_positions.npy'%self.save_data_path)
        ...
        try: self.step_x = np.nan_to_num(np.round((self.positions[:,0][end_idx]-self.positions[:,0][origin_idx])/(len(self.x)-1),3), nan = 1.)
        except: self.step_x = 1.
        try: self.step_y = np.nan_to_num(np.round((self.positions[:,1][end_idx]-self.positions[:,1][origin_idx])/(len(self.y)-1),3), nan = 1.)
        except: self.step_y = 1.
        try: self.step_z = np.nan_to_num(np.round((self.positions[:,2][end_idx]-self.positions[:,2][origin_idx])/(len(self.z)-1),3), nan = 1.)
        except: self.step_z = 1.
        # print('step_sizes:\t', self.step_x, self.step_y, self.step_z)
        # print('len(self.x,y,z.):\t', len(self.x), len(self.y), len(self.z))
        # print('self.x,y,z.shape:\t', self.x.shape, self.y.shape, self.z.shape)
        # print('self.x,y,z:\t', self.x, self.y, self.z)
        ### handle data type specific exeptions
        if type(self.parameters[0]) != np.float64 and load_type not in ['angle_file','file']:
            self.parameters = self.parameters[0]
        try: 
            self.counts = self.counts.reshape(self.positions)
        except: 
            # try: 
            #     self.len_x, self.len_y, self.len_z = np.load('%s/position_dimension.npy'%self.save_data_path)
            # except: 
            #     self.len_x, self.len_y, self.len_z = len(self.x), len(self.y), len(self.z)
        ...
        # self.sum_spec = np.sum(np.array(list(self.spectra.values())), axis = 0)  
 ...
 #       self.button_save_selection.hide()
    ...
    def plot_results(self):
        ...
        if self.one_dim:
            ...
            # aspect = np.divide(max(x_dim,y_dim), min(x_dim,y_dim))
  
        line = self.line.replace('Ka', u'K\u03B1').replace('Kb', u'K\u03b2')
        if not self.plot_ROI:
            if self.ROI_plotted == True:
                ...
                if self.rotation == 'xy':
                    self.plot_ROI = self.ax_canvas_ROI.imshow(np.rot90(self.results[:,:,layer],k = -1),
                                                              ...
                                                              extent = [self.x[-1]+self.step_x/2,self.x[0]-self.step_x/2,
                                                                        self.y[-1]+self.step_y/2,self.y[0]-self.step_y/2])
                if self.rotation == 'xz':
                    self.plot_ROI = self.ax_canvas_ROI.imshow(np.rot90(self.results[:,layer,:],k = -1),
                                                              vmin = 0, 
                                                              vmax = maximum, aspect = aspect,
                                                              extent = [self.x[-1]+self.step_x/2,self.x[0]-self.step_x/2,
                                                                        self.z[-1]+self.step_z/2,self.z[0]-self.step_z/2])
                if self.rotation == 'yz':
                    self.plot_ROI = self.ax_canvas_ROI.imshow(np.rot90(self.results[layer,:,:],k = -1),
                                                              vmin = 0, 
                                                              vmax = maximum, aspect = aspect,
                                                              extent = [self.y[-1]+self.step_y/2,self.y[0]-self.step_y/2,
                                                                        self.z[-1]+self.step_z/2,self.z[0]-self.step_z/2])
            else:
    ...
    def spectra_array(self):
        ...
        # spectra_array = np.empty([len(self.spectra) ,len(self.energy)], dtype = float )
        # for i in self.spectra:
        #         spectra_array[i]= self.spectra[i]
    ...
    def show_colormap(self):
        ...
        if self.one_dim :
          ...
          #  self.ax_canvas_ROI.set_ylabel('angle')
            ...
            if self.log_box.checkState()==2:
                ...
                self.plot_histo = self.ax_canvas_ROI.imshow(self.spectra_array(), vmin = 0, origin='lower', extent=extent, aspect=aspect)  
            ...
            #self.plot_histo = self.ax_canvas_ROI.text(0.5, 0.5, 'only for angles')
            self.figure_ROI.delaxes(self.ax_canvas_ROI)
        self.canvas_ROI.draw()
        ...
        if event.dblclick:
            ...
            if self.radio_xy.isChecked():
                if size_hor != 1: 
                    ...
                    # x_rect, y_rect = np.mgrid[int(x-(size_hor)/2)+1:int(x+(size_hor)/2)+1:self.step_x,
                    #                           int(y-size_ver/2)+1:int(y+size_ver/2)+1:self.step_y]
            ...
            if type(self.spec_rect) != np.ndarray:
                self.spec_rect = int(self.spec_rect)
            else:
                self.spec_rect = self.spec_rect.astype(np.int32)
            # except:
            #     print('please press \'show ROI\' first') 
            # print('self.len_x,Y,Z:\t', self.len_x, len_Y, len_Z)
            # print('x,y,z:\t', x, y, z)
            # print('x_rect,y,z:\t', x_rect, y_rect, z_rect)
            # print('size ver hor:\t', size_ver, size_hor)
            # print('self.x,y,z:\t', self.x, self.y, self.z)
            # print('step.x,y,z:\t', self.step_x, self.step_y, self.step_z)
            # print('positions:\t', self.positions)
            # print('where:\t', np.where((self.positions==np.array([x,y,z])).all(1)))   
            # print('spec-#:\t', self.spec_nr)
            # print('spec-rect:\t', self.spec_rect)
            # if size_hor == 1:
            #     self.spec_nr = int(np.where((self.positions==np.array([x,y,z])).all(1))[0])
            if self.load_type == 'angle_file': pass
            else:
                # self.parent.ax_canvas_spectrum.clear()
                if self.plot_style_str == 'linear': self.plot_style = self.parent.ax_canvas_spectrum.plot
                elif self.plot_style_str == 'log': self.plot_style = self.parent.ax_canvas_spectrum.semilogy
                ...
                if self.load_type == 'angle_file':
                    ...
                    self.entry_position.setText('%s'%(np.around(self.angles[self.spec_nr],decimals = 6)))
                else:
        ...
        if not event.dblclick and ((time-self.time)>0.5):
            ...
            for artist in artists[:4]:
                if type(artist) == rectangle_type:
                    artist.remove()
            ''' 
            here it is possible to draw a rectangle from the starting point
            to the button release point
            '''
            x, y, z = self.retrieve_xy_from_click(event)
            x = np.sort([x, self.x0])
            y = np.sort([y, self.y0])
            z = np.sort([z, self.z0])
            # print('drawing rectangle x,y,z:\t', x,y,z)
            if self.radio_xy.isChecked():
                ...
                # if np.diff(x) != 1: 
                    ...
                    # print('x_rect y_rect:\t', x_rect, y_rect)
            ...
            if self.load_type == 'angle_file': pass
                ...
                self.parent.ax_canvas_spectrum.clear()
                if self.plot_style_str == 'linear': self.plot_style = self.parent.ax_canvas_spectrum.plot
                if self.plot_style_str == 'log': self.plot_style = self.parent.ax_canvas_spectrum.semilogy
    ...
    def calc_rect_sum_spec(self, keys):
        ...
        '''
        # print('I am called with:\t', keys)
        len_keys = len(keys)
        for i, spec_nr in enumerate(keys):
            if i == 0:
                rect_sum_spec = np.copy(self.spectra[str(spec_nr)])
            else:
                rect_sum_spec += self.spectra[str(spec_nr)]
    ...
    def save_selection(self):
        ...
        self.save_selection_path = QtWidgets.QFileDialog().getSaveFileName(self, 
                                                                           'select save path',
                                                                           self.parent.data.file_path.replace(self.parent.data.file_type,'_selection.npy'))[0]
        spectra = []
        shape = self.spec_rect.shape
        for x in range(len(self.spec_rect)):
            for spec_nr in self.spec_rect[x]:
                spectra.append(self.spectra[str(spec_nr)])
        np.save(self.save_selection_path, np.asarray(spectra).reshape(shape+self.spectra[str(spec_nr)].shape))
    ...
    def save_ROI(self):
        ...
        self.save_ROI_path = QtWidgets.QFileDialog().getSaveFileName(self, 
                                                                     'select save path',
                                                                     self.parent.data.file_path.replace(self.parent.data.file_type,'_%s_%s_ROI.npy'%(self.element_str, self.line)))[0]
