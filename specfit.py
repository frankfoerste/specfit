...
import sys
import os # Module which supplies operating system functions
import gc
...
import natsort as ns # Module for human sorting
from functools import partial # Module to send arguments to functions via QPushButton
import h5py
import traceback
...
##############################################################################
''' import of self created Modules '''
##############################################################################

from fit_range_popup import fit_threshold_popup
from show_ROI import show_ROI
...
from display_meas_points import display_meas_points as dmp
from periodic_table import periodic_table as PSE
...
##############################################################################
class ro_database(object):
    ''' This class includes all database readout processes
    '''
    
    def __init__(self):
        '''initialized and execute complete readout'''
    ...
    def read_elements(self):
        self.elements = []
        with open(self.working_dir+'/Data/elements.dat','r') as element_file:
    ...
    def read_lines(self):
        self.lines = []
        with open(self.working_dir+'/Data/lines.dat','r') as lines_file:
            for line in lines_file:
                line = line.replace('\n','').split('\t')
                self.lines.append([line[0],line[1]])
    
    
    ### here the structure of PSE is read out of the file PSE.dat
    def read_PSE(self):
        self.PSE = []
        with open(self.working_dir+'/Data/PSE.dat','r') as PSE_file:
            for line in PSE_file:
                line = line.replace('\n','').split('\t')
    ...
    def read_char_lines(self):
        ...
        with open(self.working_dir+'/Data/characteristic_lines.dat','r') as char_lines_file:
            for line in char_lines_file:
                line = line.split()
    ...
    def read_lineE(self):  
        self.lineE = {}
        with open(self.working_dir+'/Data/lineE.dat', 'r') as lineE_file:
            for line in lineE_file:
                line = line.split()
                self.lineE[float(line[0])] = [line[1], int(line[2]), line[3], int(line[4]), float(line[5])]
...
class specFit_GUI_main(QtWidgets.QMainWindow):
...
#        self.bg_color = 'light grey'   
        ...
        self.popup_properties = show_measurement_properties.show_props(os.getcwd() + '/example_measurements/spx/1d/', self)
...
#                           +'border-radius: 1px;'\
        ...
        self.__init__plot()
        self.__init__layout()
        self.sfs = sfs.specfit_fit_settings(_parent = self)
    ...
    def __init__UI(self):  
        ...
        self.action_load_folder = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/folder-blue-open-icon.png'),'load folder',self)
        ...
        self.action_load_file = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/file-open.png'),'load file',self)
        ...
        self.action_load_angle = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/angle-open.png'),'load angle-file',self)
        ...
        self.action_load_settings = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/settings-open.png'),'load setting-file',self)
        ...
        self.action_save_settings = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/settings-save.png'),'save settings',self)
        ...
        self.action_fit_settings = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/settings-save.png'),'set fit settings',self)
        ...
        self.action_check_fit = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/check-fit.png'),'check fit',self)
        ...
        self.action_ROI_image = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/show-ROI.png'),'view ROI',self)
        ...
        self.action_max_pixel_spec = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/video-display.png'),'maximum pixel spectrum',self)
#        self.action_max_pixel_spec.setShortcut('F6')
        ...
        self.action_show_plot3d = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/plot-3d.png'),'plot 3D',self)
        ...
        self.action_show_display_meas_points = QtWidgets.QAction('display meas points 3D',self)
#        self.action_show_display_meas_points.setShortcut('F8')
        ...
        self.action_clear_plot = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/clear-plot.png'),'clear plot',self)
        ...
        self.action_clear_element_lines = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/clear-elements.png'),'clear elements and lines',self)
        ...
        self.action_fit = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/fit-and-save.png'),'fit and save',self)
        self.action_fit.setShortcut('F1')
        self.action_fit.setStatusTip('fit and save')
        self.action_fit.triggered.connect(self.fit_folder)

        self.action_npy2bin = QtWidgets.QAction('convert .npy to .bin',self)
        self.action_npy2bin.setStatusTip('display measurement points 3D')
        self.action_npy2bin.triggered.connect(self.npy2bin)

        self.action_export2npz = QtWidgets.QAction('export .npy',self)
        self.action_export2npz.setStatusTip('export measurement to .npy')
        self.action_export2npz.triggered.connect(self.export2npz)
        
        self.action_split_datah5 = QtWidgets.QAction('split data.h5',self)
        self.action_split_datah5.setStatusTip('split the data.h5 to fit in batches')
        self.action_split_datah5.triggered.connect(self.splith5)
        
        self.action_h52tiff = QtWidgets.QAction('results.h5 to .tiff',self)
        self.action_h52tiff.setStatusTip('transform results.h5 to tiff images')
        self.action_h52tiff.triggered.connect(self.h52tiff)
        
        self.action_show_counts = QtWidgets.QAction('show counts',self)
        ...
        self.action_ipython_console = QtWidgets.QAction('start ipython console',self)
        ...
        self.action_exit = QtWidgets.QAction(QtGui.QIcon(self.working_directory+'/Data/icons/exit.png'),'Exit',self)
        ...
        self.menu_export.addAction(self.action_export2npz)
        self.menu_export.addAction(self.action_npy2bin)
        self.menu_export.addAction(self.action_split_datah5)
        self.menu_export.addAction(self.action_h52tiff)
        
        self.menu_export = self.menubar.addMenu('&Edit')
        self.menu_export.addAction(self.action_ipython_console)
...
#        self.energy_and_para_widget.set
...
#        self.label_logo.setPixmap(QtGui.QPixmap(os.getcwd() + '/Data/images/specfit_logo_vertical.png'))
...
#        self.label_logo.show()       
        ...
        self.check_calc_minima.setCheckState(2)
        ...
        self.combo_lib.addItems(['xraylib', 'TUB'])
...
#        self.PSE_widget.move(5, 9*self.row_height)
    ...
    def __init_connections(self, ):
        ...
        self.check_calc_minima.stateChanged.connect(lambda state: self.sfs.check_calc_minima.setChecked(state == QtCore.Qt.Checked))
    ...
    def load_folder(self):
        try:
            ...
            if self.data.file_type == '.bcf':
                self.s.life_time = self.data.parameters[4]  # was changed from self.s.life_time = self.parameters[0][4]
            else:
                self.s.life_time = self.data.parameters[0][4]
            try: self.s.real_time = self.data.parameters[0][7] #!TODO real time has to be implemented for every file format
            except: pass 
    ...
    def ipython_console(self,):
        ...
        # self.setCentralWidget(self.ipython_widget)
    ...
    def reset_2_default(self):
        ...
        self.sfs.check_batch_fitting.setChecked(self.batch_fitting)
    ...
    def open_param_file(self):    
        '''
        this function opens a display to select a file and set it's parameters
        '''
        self.PSE_widget.tab_udl.reset_2_default()
        ...
        try:
            ...
            self.statusBar().showMessage(f'file could not be loaded: {param_path}.')
        self.check_fit()
    ...
    def store_GUI_values(self): 
        ''' updates data_handler with GUI_values '''
        ...
        self.data.use_parameters = (self.check_use_parameters.checkState() == 2)
        self.data.bg_zero = (self.check_set_bg_zero.checkState() == 2)
        ...
        else: self.data.strip_width = int(self.entry_strip_width.text())
        ...
        self.data.nl_fit = (self.check_nl_fit.checkState() ==2) 
        self.data.calc_minima = (self.check_calc_minima.checkState() ==2) 
    ...
    def display_values_in_GUI(self):
        ''' This function displays the values from the data class in GUI '''
    ...
    def show_sum_spec(self): # TODO this is in principle the same as plot_canvas
        ''' This function displays a sum spectrum of the loaded spectra. '''
        ...
        # try:
        #     self.popup_properties.fill_text(u'steps width: {:d} µm,{:d} µm,{:d} µm \n'.format(*(self.data.step_width*1000).astype(int)))
        # except: 
        #     print('step width could not be estimated')
        # if type(self.data.position_dimension[0]) != np.ndarray:
        #     self.popup_properties.fill_text('position steps: {:d}-{:d}-{:d} \n'.format(*self.data.position_dimension))
        #     try:
        #         self.popup_properties.fill_text(u'steps width: {:d} µm,{:d} µm,{:d} µm \n'.format(*(self.data.step_width*1000).astype(int)))
        #     except: pass
        # else:
        #     print('tensor data is multidimensional (.mca)')
        ### set main GUI on top
        self.activateWindow()
    ...
    def fill_and_display_show_ROI(self):
         '''load current  params in ROI widget and display it '''
         # self.check_fit()
    ...
    def load_parameter_in_specfit_deconvolution(self, spectra_nr = 0):
        ...
        minchange (nl_fit threshold) and database (TUB or xraylib) from the 
        data_handler into specfit_deconvolution
        '''  
    ...
    def get_selected_parameters(self):
        ...
        if self.data.use_parameters == False and self.data.loadtype in ['file','msa_file','angle_file', 
                                                                        'hdf5_file', 'bcf_file', 'csv_file']:
            parameters = self.data.parameters
        elif self.data.use_parameters == False and self.data.loadtype in ['folder']:
            if self.data.file_type == '.bcf':
                parameters = self.data.parameters
            else:
                parameters = self.data.parameters[self.ROI_widget.spec_nr]
        elif self.data.use_parameters == True:
    ...
    def load_spec_in_specfit_deconvolution(self, spectra_nr = None):
        '''
        loads a spectrum in specfit_deconvolution depending on data_handler.load_type 
        ...
        negative spectra_nr means data_handler.sum_spec 
    ...
    def export2npz(self, ):
        ...
        try: self.data.loadtype
            ...
            return
        export2npz_path = QtWidgets.QFileDialog().getSaveFileName(self, 
                                                                  'select save path',
                                                                  self.data.file_path.replace(self.data.file_type,'.npz'))[0]
        if '.npz' not in export2npz_path:
            export2npz_path += '.npz'
        ### now read out data
        ...
        np.savez_compressed(export2npz_path, data = spectra)
        self.statusBar().showMessage('export done')


    def clear_ax_canvas_spectrum(self, labels = []):
        ''' 
        ...
        if labels == []:
            ...
            for line in list(self.ax_canvas_spectrum.get_lines()):
                line.remove()
            if self.ax_canvas_spectrum.get_legend() != None:
    ...
    def plot_canvas(self, spectrum, energy, background, calc_spec):
        ...
        if self.plot_style != None:
            ...
            try: 
                if self.data.plot_style == 'lin':
                    self.ax_canvas_spectrum.set_ylim(0, np.max(spectrum[plot_start:plot_end])*1.1)
                else:
                    self.ax_canvas_spectrum.set_ylim(np.min(spectrum[plot_start:plot_end])*0.9, np.max(spectrum[plot_start:plot_end])*1.1)
            except: 
                if self.data.plot_style == 'lin':
                    self.ax_canvas_spectrum.set_ylim(0, np.max(spectrum[plot_start:plot_end])*1.1)
                else:
    ...
    def clear_plot(self):
        ...
        self.clear_ax_canvas_spectrum()
        self.s.Strip = None
        self.ROI_widget.spec_nr = -1 #reset to sum sppec
        if self.plot_style != None:
            plot_start, plot_end = self.data.get_ROI_indicees()
    ...
    def fit_folder(self):
        ...
        def create_empty_results(load_type = 'folder',
            ...
            if load_type in ['file', 'angle_file']:
                ...
                for element, linelist in zip(selected_elements, selected_lines):
                    for line in linelist:
                        if type(dimension[0]) != type(np.ndarray(0)):
            ...
            for key in results.keys():
                ...
                if save_storage == '.h5':
                    with h5py.File(f'{save_path}/results.h5', 'a') as tofile:
                        ...
                        if results_key in list(tofile.keys()):
        ...
        def show_fit_prop_in_popup():   ### write fit-lines to properties-popup
            ...
            if self.data.use_parameters == True:
        ...
        #fit every spectrum
        if self.data.loadtype == 'file': #means only one spectra
            self.check_nl_fit.setChecked(False)
            ...
            # for i, angle in enumerate(self.data.spectra):  
            ...
            for data_batch in range(data_batches):
                ...
                self.batch_fitting = self.sfs.check_batch_fitting.checkState()
                ### check the type of save_storage
                if self.sfs.check_save_storage.checkState():
                    save_storage = '.npy'
                else:
                    save_storage = '.h5'
                ...
                ### sorted spectra dict
                if type(self.data.position_dimension[0]) != type(np.ndarray(0)):
                ...
                for i in range(partitions):
                    ...
                    if i == 1: self.s.fit_in_progress = True
                    ...
                    try: cur_spectra =  self.data.spectra[i]         
                    except IndexError: cur_spectra = np.load(f'{self.folder_path}/single_spectra/spectrum_{i}.npy')
                                ...
                                for scan, sum_len_scan in enumerate(self.data.sum_len_scans):
                                    if i in sum_len_scan:
                                        ...
                                        if len(self.data.len_scans) > 1:
                                            index_0 = self.data.tensor_positions[scan][0][i-np.sum(self.data.len_scans[:scan]),0]
                                            index_1 = self.data.tensor_positions[scan][0][i-np.sum(self.data.len_scans[:scan]),1]
                                            index_2 = self.data.tensor_positions[scan][0][i-np.sum(self.data.len_scans[:scan]),2]
                                        else:
                                        ...
                                        results[element_key][index_0][index_1][index_2] = getResults['_'.join(key.split('_')[:-2])]['I']
                                        continue
                    else:
                        for key in self.s.get_result_keys():
                            results[key][self.data.tensor_positions[i][0]][self.data.tensor_positions[i][1]][self.data.tensor_positions[i][2]] = 0
                        continue
                    ...
                    self.statusBar().showMessage('progress : batch %d/%d - %d/%d - %.2f %% - time to go: %s'%(data_batch+1, len(datah5_files), i+1,partitions,(i+1)/float(partitions)*100,time.strftime('%H:%M:%S', time2go)))
                    QtWidgets.QApplication.processEvents()
...
def main():
    ...
    for module in module_list:
        splash.showMessage(f'{module} is set up', QtCore.Qt.AlignBottom, QtCore.Qt.red), app.processEvents(), time.sleep(np.random.uniform(0.,0.3))
    splash.showMessage('done', QtCore.Qt.AlignBottom, QtCore.Qt.red), app.processEvents()
    
    specFit_GUI = specFit_GUI_main()
